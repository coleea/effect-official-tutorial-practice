...
export const flatten = self => {
  ...
  while (1) {
    const [parallel, sequential] = List.reduce(current, [parallelCollectionEmpty(), List.empty()], ([parallel, sequential], blockedRequest) => {
      ...
      return [parallelCollectionCombine(parallel, par), List.concat(sequential, seq)];
    });
...
//
/** @internal */
export const EntryTypeId = /*#__PURE__*/Symbol.for("@effect/io/RequestBlock.Entry");
/** @internal */
class EntryImpl {
...
/** @internal */
export const isEntry = u => {
  return typeof u === "object" && u != null && EntryTypeId in u;
};
/** @internal */
export const makeEntry = (request, result, listeners, ownerId, state) => new EntryImpl(request, result, listeners, ownerId, state);
/** @internal */
export const RequestBlockParallelTypeId = /*#__PURE__*/Symbol.for("@effect/io/RequestBlockParallel");
...
/** @internal */
export const parallelCollectionMake = (dataSource, blockedRequest) => new ParallelImpl(HashMap.make([dataSource, Array.of(blockedRequest)]));
/** @internal */
export const parallelCollectionCombine = (self, that) => new ParallelImpl(HashMap.reduceWithIndex(self.map, that.map, (map, value, key) => HashMap.set(map, key, Option.match(() => value, a => [...a, ...value])(HashMap.get(map, key)))));
/** @internal */
...
/** @internal */
export const parallelCollectionToSequentialCollection = self => sequentialCollectionMake(HashMap.map(self.map, x => Array.of(x)));
/** @internal */
export const parallelCollectionToChunk = self => Array.from(self.map);
/** @internal */
export const SequentialCollectionTypeId = /*#__PURE__*/Symbol.for("@effect/io/RequestBlockSequential");
/** @internal */
...
/** @internal */
export const sequentialCollectionCombine = (self, that) => new SequentialImpl(HashMap.reduceWithIndex(that.map, self.map, (map, value, key) => HashMap.set(map, key, Option.match(() => [], a => [...a, ...value])(HashMap.get(map, key)))));
/** @internal */
