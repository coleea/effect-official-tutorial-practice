 ...
 * @internal
 */
export const currentRequestBatchingEnabled = /*#__PURE__*/globalValue( /*#__PURE__*/Symbol.for("@effect/io/FiberRef/currentRequestBatchingEnabled"), () => core.fiberRefUnsafeMake(true));
/**
...
const runBlockedRequests = (self, id) => core.forEachDiscard(_RequestBlock.flatten(self), requestsByRequestResolver => forEachParDiscard(_RequestBlock.sequentialCollectionToChunk(requestsByRequestResolver), ([dataSource, sequential]) => {
  ...
  return core.fiberRefLocally(core.flatMap(core.flatMap(forkDaemon(core.interruptible(dataSource.runAll(sequential))), processing => core.asyncInterrupt(cb => {
    ...
    return core.forEachDiscard(residual, entry => complete(entry.request, core.exitInterrupt(id)));
...
export class FiberRuntime {
  ...
  [OpCodes.OP_FAILURE](op) {
    ...
    if (internalCause.isAnnotatedType(cause) && internalCause.isStackAnnotation(cause.annotation)) {
      ...
      cause = internalCause.annotated(op.i0, new StackAnnotation(this.stackToLines(), MRef.getAndIncrement(internalCause.globalErrorSeq)));
    }
...
/** @internal */
export const defaultLogger = /*#__PURE__*/internalLogger.makeLogger((fiberId, logLevel, message, cause, context, spans, annotations) => {
...
/** @internal */
export const filterMinimumLogLevel = /*#__PURE__*/internalLogger.makeLogger((fiberId, logLevel, message, cause, context, spans, annotations) => {
  const formatted = internalLogger.stringLogger.log(fiberId, logLevel, message, cause, context, spans, annotations);
  globalThis.console.log(formatted);
});
/** @internal */
export const logFmtLogger = /*#__PURE__*/internalLogger.makeLogger((fiberId, logLevel, message, cause, context, spans, annotations) => {
...
/* @internal */
export const acquireRelease = /*#__PURE__*/Debug.dualWithTrace(2, (trace, restore) => (acquire, release) => core.uninterruptible(core.tap(acquire, a => addFinalizer(exit => restore(release)(a, exit)))).traced(trace));
/* @internal */
...
/* @internal */
export const allParDiscard = /*#__PURE__*/Debug.methodWithTrace(trace => function () {
  if (arguments.length === 1) {
    if (core.isEffect(arguments[0])) {
      ...
      return forEachParDiscard(arguments[0], identity).traced(trace);
    } else {
      return forEachParDiscard(Object.entries(arguments[0]), ([_, e]) => core.asUnit(e)).traced(trace);
    }
  }
  return forEachParDiscard(arguments, identity).traced(trace);
});
/* @internal */
export const collectAllSuccessesPar = /*#__PURE__*/Debug.methodWithTrace(trace => elements => allFilterMapPar(Array.from(elements).map(core.exit), exit => core.exitIsSuccess(exit) ? Option.some(exit.i0) : Option.none()).traced(trace));
/* @internal */
export const allFilterMapPar = /*#__PURE__*/Debug.dualWithTrace(2, (trace, restore) => (elements, pf) => core.map(allPar(elements), RA.filterMap(restore(pf))).traced(trace));
/* @internal */
export const collectAllPar = /*#__PURE__*/Debug.methodWithTrace(trace => elements => core.map(allPar(elements), RA.filterMap(identity)).traced(trace));
/* @internal */
...
/* @internal */
export const filterPar = /*#__PURE__*/Debug.dualWithTrace(2, (trace, restore) => (elements, f) => core.map(RA.compact)(forEachPar(elements, a => core.map(restore(f)(a), b => b ? Option.some(a) : Option.none()))).traced(trace));
/* @internal */
...
/* @internal */
export const forEachExec = /*#__PURE__*/Debug.dualWithTrace(3, (trace, restore) => (elements, f, strategy) => core.suspend(() => ExecutionStrategy.match(() => core.forEach(restore(f))(elements), () => core.withParallelism("unbounded")(forEachPar(restore(f))(elements)), parallelism => core.withParallelism(parallelism)(forEachPar(restore(f))(elements)))(strategy)).traced(trace));
/* @internal */
...
/* @internal */
export const forEachParWithIndex = /*#__PURE__*/Debug.dualWithTrace(2, (trace, restore) => (elements, f) => core.suspend(() => core.flatMap(core.sync(() => []), array => core.map(forEachParDiscard(Array.from(elements).map((a, i) => [a, i]), ([a, i]) => core.flatMap(core.suspend(() => restore(f)(a, i)), b => core.sync(() => {
  array[i] = b;
}))), () => array))).traced(trace));
/* @internal */
...
/* @internal */
export const reduceAllPar = /*#__PURE__*/Debug.dualWithTrace(3, (trace, restore) => (elements, zero, f) => core.suspend(() => core.map(option => {
  switch (option._tag) {
    case "None":
      {
...
/* @internal */
export const scope = /*#__PURE__*/Debug.methodWithTrace(trace => () => scopeTag.traced(trace));
/* @internal */
export const scopeWith = /*#__PURE__*/Debug.methodWithTrace((trace, restore) => f => core.flatMap(scopeTag, restore(f)).traced(trace));
/* @internal */
...
/* @internal */
export const someWith = /*#__PURE__*/Debug.dualWithTrace(2, (trace, restore) => (self, f) => core.suspend(() => unsome(restore(f)(some(self)))).traced(trace));
/* @internal */
export const allPar = /*#__PURE__*/Debug.methodWithTrace(trace => function () {
  if (arguments.length === 1) {
    if (core.isEffect(arguments[0])) {
      return core.map(arguments[0], x => [x]).traced(trace);
    } else if (Array.isArray(arguments[0]) || Symbol.iterator in arguments[0]) {
      return forEachPar(arguments[0], identity).traced(trace);
    } else {
  ...
  return forEachPar(arguments, identity).traced(trace);
});
/* @internal */
...
/* @internal */
export const validateAllPar = /*#__PURE__*/Debug.dualWithTrace(2, (trace, restore) => (elements, f) => core.flatMap(partitionPar(elements, restore(f)), ([es, bs]) => es.length === 0 ? core.succeed(bs) : core.fail(es)).traced(trace));
/* @internal */
export const validateAllParDiscard = /*#__PURE__*/Debug.dualWithTrace(2, (trace, restore) => (elements, f) => core.flatMap(partitionPar(elements, restore(f)), ([es, _]) => es.length === 0 ? core.unit() : core.fail(es)).traced(trace));
/* @internal */
...
/* @internal */
export const withEarlyRelease = /*#__PURE__*/Debug.methodWithTrace(trace => self => scopeWith(parent => core.flatMap(core.scopeFork(parent, ExecutionStrategy.sequential), child => core.map(value => tuple(core.fiberIdWith(fiberId => core.scopeClose(child, core.exitInterrupt(fiberId))), value))(scopeExtend(child)(self)))).traced(trace));
/* @internal */
...
/** @internal */
export const disconnect = /*#__PURE__*/Debug.methodWithTrace(trace => self => core.uninterruptibleMask(restore => core.fiberIdWith(fiberId => core.flatMap(forkDaemon(restore(self)), fiber => core.onInterrupt(() => internalFiber.interruptAsFork(fiberId)(fiber))(restore(internalFiber.join(fiber)))))).traced(trace));
...
/** @internal */
export const raceAwait = /*#__PURE__*/Debug.dualWithTrace(2, trace => (self, that) => core.fiberIdWith(parentFiberId => raceWith(that, (exit, right) => core.exitMatchEffect(cause => mapErrorCause(cause2 => internalCause.parallel(cause, cause2))(internalFiber.join(right)), value => core.as(value)(core.interruptAsFiber(parentFiberId)(right)))(exit), (exit, left) => core.exitMatchEffect(cause => mapErrorCause(cause2 => internalCause.parallel(cause2, cause))(internalFiber.join(left)), value => core.as(value)(core.interruptAsFiber(parentFiberId)(left)))(exit))(self)).traced(trace));
/** @internal */
