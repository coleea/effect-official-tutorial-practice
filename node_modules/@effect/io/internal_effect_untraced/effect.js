...
const absolveWith = /*#__PURE__*/Debug.dualWithTrace(2, (trace, restore) => (self, f) => core.flatMap(value => restore(f)(value))(self).traced(trace));
/* @internal */
...
const allDiscard = /*#__PURE__*/Debug.methodWithTrace(trace => function () {
  if (arguments.length === 1) {
    if (core.isEffect(arguments[0])) {
      ...
      return core.forEachDiscard(arguments[0], _Function.identity).traced(trace);
    } else {
      return core.forEachDiscard(Object.entries(arguments[0]), ([_, e]) => core.asUnit(e)).traced(trace);
    }
  }
  return core.forEachDiscard(arguments, _Function.identity).traced(trace);
});
/* @internal */
exports.allDiscard = allDiscard;
const allFilterMap = /*#__PURE__*/Debug.dualWithTrace(2, (trace, restore) => (elements, pf) => core.map(all(elements), RA.filterMap(restore(pf))).traced(trace));
/* @internal */
exports.allFilterMap = allFilterMap;
const collectAll = /*#__PURE__*/Debug.methodWithTrace(trace => elements => core.map(all(elements), RA.filterMap(_Function.identity)).traced(trace));
/* @internal */
exports.collectAll = collectAll;
const filterMapEffect = /*#__PURE__*/Debug.dualWithTrace(2, (trace, restore) => (elements, f) => {
...
const collectAllSuccesses = /*#__PURE__*/Debug.methodWithTrace(trace => as => allFilterMap(exit => Exit.isSuccess(exit) ? Option.some(exit.i0) : Option.none())(Array.from(as).map(core.exit)).traced(trace));
/* @internal */
...
/* @internal */
exports.bind = bind;
const bindDiscard = /*#__PURE__*/Debug.dualWithTrace(3, trace => (self, tag, f) => core.flatMap(self, k => core.map(f, a => ({
  ...k,
  [tag]: a
}))).traced(trace));
/* @internal */
...
const bindValueDiscard = /*#__PURE__*/Debug.dualWithTrace(3, trace => (self, tag, f) => core.map(self, k => ({
  ...k,
  [tag]: f
})).traced(trace));
/* @internal */
exports.bindValueDiscard = bindValueDiscard;
...
const filter = /*#__PURE__*/Debug.dualWithTrace(2, (trace, restore) => (elements, f) => core.suspend(() => Array.from(elements).reduceRight((effect, a) => core.zipWith(effect, core.suspend(() => restore(f)(a)), (list, b) => b ? [a, ...list] : list), core.sync(() => new Array()))).traced(trace));
/* @internal */
...
const filterOrDieMessage = /*#__PURE__*/Debug.dualWithTrace(3, (trace, restore) => (self, f, message) => filterOrElse(self, restore(f), () => core.dieMessage(message)).traced(trace));
/* @internal */
...
const forEachWithIndex = /*#__PURE__*/Debug.dualWithTrace(2, (trace, restore) => (elements, f) => core.suspend(() => {
  let index = 0;
  const acc = [];
  return core.map(() => acc)(core.forEachDiscard(elements, a => core.map(restore(f)(a, index), b => {
    acc.push(b);
    index++;
  })));
}).traced(trace));
...
const loop = /*#__PURE__*/Debug.methodWithTrace((trace, restore) => (initial, cont, inc, body) => core.map(loopInternal(initial, restore(cont), restore(inc), restore(body)), x => Array.from(x)).traced(trace));
exports.loop = loop;
...
const mapAccum = /*#__PURE__*/Debug.dualWithTrace(3, (trace, restore) => (elements, zero, f) => core.suspend(() => {
  ...
  return core.map(result, z => (0, RA.tuple)(z, builder));
}).traced(trace));
/* @internal */
...
/* @internal */
const replicate = n => self => Array.from({
  length: n
}, () => self);
/* @internal */
exports.replicate = replicate;
const replicateEffect = /*#__PURE__*/Debug.dualWithTrace(2, trace => (self, n) => all(replicate(n)(self)).traced(trace));
/* @internal */
exports.replicateEffect = replicateEffect;
const replicateEffectDiscard = /*#__PURE__*/Debug.dualWithTrace(2, trace => (self, n) => allDiscard(replicate(n)(self)).traced(trace));
/* @internal */
...
const summarized = /*#__PURE__*/Debug.dualWithTrace(3, (trace, restore) => (self, summary, f) => core.flatMap(summary, start => core.flatMap(self, value => core.map(summary, end => (0, RA.tuple)(restore(f)(start, end), value)))).traced(trace));
/* @internal */
...
const attemptCatchPromise = /*#__PURE__*/Debug.methodWithTrace((trace, restore) => (evaluate, onReject) => core.flatMap(attemptCatch(restore(evaluate), restore(onReject)), promise => core.async(resolve => {
...
const attemptCatchPromiseInterrupt = /*#__PURE__*/Debug.methodWithTrace((trace, restore) => (evaluate, onReject) => core.flatMap(attemptCatch(() => {
  const controller = new AbortController();
  return [controller, restore(evaluate)(controller.signal)];
}, restore(onReject)), ([controller, promise]) => core.asyncInterrupt(resolve => {
  promise.then(a => resolve(core.exitSucceed(a))).catch(e => resolve(core.exitFail(restore(onReject)(e))));
  return core.sync(() => controller.abort());
})).traced(trace));
/* @internal */
exports.attemptCatchPromiseInterrupt = attemptCatchPromiseInterrupt;
const attemptPromise = /*#__PURE__*/Debug.methodWithTrace((trace, restore) => evaluate => core.flatMap(restore(attempt)(evaluate), promise => core.async(resolve => {
  promise.then(a => resolve(core.exitSucceed(a))).catch(e => resolve(core.exitFail(e)));
})).traced(trace));
/* @internal */
exports.attemptPromise = attemptPromise;
const attemptPromiseInterrupt = /*#__PURE__*/Debug.methodWithTrace((trace, restore) => evaluate => core.flatMap(attempt(() => {
  const controller = new AbortController();
  return [controller, restore(evaluate)(controller.signal)];
...
const all = /*#__PURE__*/Debug.methodWithTrace(trace => function () {
  if (arguments.length === 1) {
    if (core.isEffect(arguments[0])) {
      ...
      return core.forEach(arguments[0], _Function.identity).traced(trace);
    } else {
  ...
  return core.forEach(arguments, _Function.identity).traced(trace);
});
/* @internal */
exports.all = all;
...
const unfold = /*#__PURE__*/Debug.methodWithTrace((trace, restore) => (s, f) => core.map(unfoldLoop(s, restore(f), Chunk.empty()), a => Array.from(Chunk.reverse(a))).traced(trace));
/* @internal */
...
const validate = /*#__PURE__*/Debug.dualWithTrace(2, trace => (self, that) => validateWith(self, that, (a, b) => (0, RA.tuple)(a, b)).traced(trace));
/* @internal */
exports.validate = validate;
const validateAll = /*#__PURE__*/Debug.dualWithTrace(2, (trace, restore) => (elements, f) => core.flatMap(partition(elements, restore(f)), ([es, bs]) => es.length === 0 ? core.succeed(bs) : core.fail(es)).traced(trace));
/* @internal */
exports.validateAll = validateAll;
const validateAllDiscard = /*#__PURE__*/Debug.dualWithTrace(2, (trace, restore) => (elements, f) => core.flatMap(partition(elements, restore(f)), ([es, _]) => es.length === 0 ? core.unit() : core.fail(es)).traced(trace));
/* @internal */
...
const whenFiberRef = /*#__PURE__*/Debug.dualWithTrace(3, (trace, restore) => (self, fiberRef, predicate) => core.flatMap(core.fiberRefGet(fiberRef), s => restore(predicate)(s) ? core.map(self, a => (0, RA.tuple)(s, Option.some(a))) : core.succeed([s, Option.none()])).traced(trace));
/* @internal */
exports.whenFiberRef = whenFiberRef;
const whenRef = /*#__PURE__*/Debug.dualWithTrace(3, (trace, restore) => (self, ref, predicate) => core.flatMap(Ref.get(ref), s => restore(predicate)(s) ? core.map(self, a => (0, RA.tuple)(s, Option.some(a))) : core.succeed([s, Option.none()])).traced(trace));
/* @internal */
