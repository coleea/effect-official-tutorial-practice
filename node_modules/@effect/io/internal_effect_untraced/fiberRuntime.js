...
exports.withRuntimeFlagsScoped = exports.withEarlyRelease = exports.withConfigProviderScoped = exports.withClockScoped = exports.validateFirstPar = exports.validateAllParDiscard = exports.validateAllPar = exports.using = exports.unsome = exports.unsafeMakeChildFiber = exports.unsafeFork = exports.taggedScopedWithLabels = exports.taggedScopedWithLabelSet = exports.taggedScoped = exports.someWith = exports.sequentialFinalizers = exports.scopedEffect = exports.scopeWith = exports.scopeUse = exports.scopeTag = exports.scopeMake = exports.scopeExtend = exports.scope = exports.runtimeFiberVariance = exports.releaseMapReleaseAll = exports.reduceAllPar = exports.raceWith = exports.raceFibersWith = exports.raceAwait = exports.raceAll = exports.race = exports.partitionPar = exports.parallelFinalizers = exports.onDoneCause = exports.onDone = exports.mergeAllPar = exports.logFmtLogger = exports.forkWithErrorHandler = exports.forkDaemon = exports.forkAllDiscard = exports.fork = exports.forEachParWithIndex = exports.forEachParDiscard = exports.forEachPar = exports.forEachExec = exports.filterPar = exports.filterNotPar = exports.filterMinimumLogLevel = exports.fiberScoped = exports.fiberRefUnsafeMakeSupervisor = exports.fiberRefMakeWith = exports.fiberRefMakeRuntimeFlags = exports.fiberRefMakeContext = exports.fiberRefMake = exports.fiberRefLocallyScopedWith = exports.fiberRefLocallyScoped = exports.fiberJoinAll = exports.fiberInterruptFork = exports.fiberCollectAll = exports.fiberAwaitAll = exports.existsPar = exports.ensuring = exports.disconnect = exports.defaultLogger = exports.daemonChildren = exports.currentSupervisor = exports.currentRuntimeFlags = exports.currentRequestBatchingEnabled = exports.currentMinimumLogLevel = exports.currentLoggers = exports.collectAllSuccessesPar = exports.collectAllPar = exports.allParDiscard = exports.allPar = exports.allFilterMapPar = exports.addFinalizer = exports.acquireRelease = exports.FiberRuntime = void 0;
...
var _completedRequestMap = /*#__PURE__*/require("@effect/io/internal_effect_untraced/completedRequestMap");
 ...
 * @internal
 */
const currentRequestBatchingEnabled = /*#__PURE__*/(0, _Global.globalValue)( /*#__PURE__*/Symbol.for("@effect/io/FiberRef/currentRequestBatchingEnabled"), () => core.fiberRefUnsafeMake(true));
/**
 * Executes all requests, submitting requests to each data source in parallel.
 */
exports.currentRequestBatchingEnabled = currentRequestBatchingEnabled;
const runBlockedRequests = (self, id) => core.forEachDiscard(_RequestBlock.flatten(self), requestsByRequestResolver => forEachParDiscard(_RequestBlock.sequentialCollectionToChunk(requestsByRequestResolver), ([dataSource, sequential]) => {
  const map = new Map();
  ...
  return core.fiberRefLocally(core.flatMap(core.flatMap(forkDaemon(core.interruptible(dataSource.runAll(sequential))), processing => core.asyncInterrupt(cb => {
    ...
    return core.forEachDiscard(residual, entry => (0, _request.complete)(entry.request, core.exitInterrupt(id)));
...
class FiberRuntime {
  ...
  ["Blocked"](op) {
    ...
    return core.uninterruptibleMask(restore => core.flatMap(fork(runBlockedRequests(op.i0, this._fiberId)), () => restore(op.i1)));
  }
  ["RunBlocked"](op) {
...
const filterMinimumLogLevel = /*#__PURE__*/internalLogger.makeLogger((fiberId, logLevel, message, cause, context, spans, annotations) => {
  const formatted = internalLogger.stringLogger.log(fiberId, logLevel, message, cause, context, spans, annotations);
  globalThis.console.log(formatted);
});
/** @internal */
...
const allParDiscard = /*#__PURE__*/Debug.methodWithTrace(trace => function () {
  if (arguments.length === 1) {
    if (core.isEffect(arguments[0])) {
      ...
      return forEachParDiscard(arguments[0], _Function.identity).traced(trace);
    } else {
      return forEachParDiscard(Object.entries(arguments[0]), ([_, e]) => core.asUnit(e)).traced(trace);
    }
  }
  return forEachParDiscard(arguments, _Function.identity).traced(trace);
});
/* @internal */
exports.allParDiscard = allParDiscard;
const collectAllSuccessesPar = /*#__PURE__*/Debug.methodWithTrace(trace => elements => allFilterMapPar(Array.from(elements).map(core.exit), exit => core.exitIsSuccess(exit) ? Option.some(exit.i0) : Option.none()).traced(trace));
/* @internal */
exports.collectAllSuccessesPar = collectAllSuccessesPar;
const allFilterMapPar = /*#__PURE__*/Debug.dualWithTrace(2, (trace, restore) => (elements, pf) => core.map(allPar(elements), RA.filterMap(restore(pf))).traced(trace));
/* @internal */
exports.allFilterMapPar = allFilterMapPar;
const collectAllPar = /*#__PURE__*/Debug.methodWithTrace(trace => elements => core.map(allPar(elements), RA.filterMap(_Function.identity)).traced(trace));
/* @internal */
...
const filterPar = /*#__PURE__*/Debug.dualWithTrace(2, (trace, restore) => (elements, f) => core.map(RA.compact)(forEachPar(elements, a => core.map(restore(f)(a), b => b ? Option.some(a) : Option.none()))).traced(trace));
/* @internal */
...
const forEachParUnboundedDiscard = (self, f) => core.suspend(() => {
  ...
  return core.uninterruptibleMask(restore => {
    ...
    let ref = 0;
    const residual = [];
    const process = core.transplant(graft => core.forEach(as, a => forkDaemon(graft(core.flatMap(exit => {
      switch (exit._tag) {
        case "Failure":
          {
            if (residual.length > 0) {
              ...
              return core.blocked(requests, core.matchCauseEffect(_continue, cause => core.zipRight(core.deferredFail(deferred, void 0), core.failCause(internalCause.parallel(cause, exit.cause))), () => core.zipRight(core.deferredFail(deferred, void 0), core.failCause(exit.cause))));
            }
...
/* @internal */
const forEachParNDiscard = (self, n, f) => n === 1 ? forEachBatchedDiscard(self, f) : core.suspend(() => {
  const iterator = self[Symbol.iterator]();
  const residual = [];
  const worker = core.flatMap(core.sync(() => iterator.next()), next => next.done ? core.unit() : core.flatMapStep(core.asUnit(f(next.value)), res => {
    switch (res._tag) {
      case "Blocked":
        {
          residual.push(res);
  ...
  return core.flatMap(core.exit(forEachParUnboundedDiscard(effects, _Function.identity)), exit => {
    if (residual.length === 0) {
      return exit;
    }
    const requests = residual.map(blocked => blocked.i0).reduce(_RequestBlock.par);
    ...
    if (exit._tag === "Failure") {
      return core.blocked(requests, core.matchCauseEffect(_continue, cause => core.exitFailCause(internalCause.parallel(exit.cause, cause)), () => exit));
    }
...
const raceAll = /*#__PURE__*/Debug.methodWithTrace(trace => all => {
  ...
  return core.flatMap(done => core.flatMap(fails => core.uninterruptibleMask(restore => core.flatMap(head => core.flatMap(fibers => core.onInterrupt(() => Chunk.reduce(core.unit(), (effect, fiber) => core.zipLeft(core.interruptFiber(fiber))(effect))(fibers))(restore(core.flatMap(inheritAll)(Deferred.await(done)))))(core.tap(fibers => Chunk.reduce(core.unit(), (effect, fiber) => core.zipRight(core.asUnit(fork(core.flatMap(raceAllArbiter(fibers, fiber, done, fails))(internalFiber._await(fiber)))))(effect))(fibers))(core.map(tail => Chunk.prepend(head)(tail))(core.map(Chunk.unsafeFromArray)(core.forEach(effect => fork(core.interruptible(effect)))(effects))))))(fork(core.interruptible(self)))))(Ref.make(effects.length)))(core.deferredMake()).traced(trace);
});
/* @internal */
exports.raceAll = raceAll;
const raceAllArbiter = (fibers, winner, deferred, fails) => exit => core.exitMatchEffect(cause => core.flatten(Ref.modify(fails, fails => [fails === 0 ? core.asUnit(core.deferredFailCause(deferred, cause)) : core.unit(), fails - 1])), value => core.flatMap(set => set ? Chunk.reduce(core.unit(), (effect, fiber) => fiber === winner ? effect : core.zipLeft(core.interruptFiber(fiber))(effect))(Chunk.fromIterable(fibers)) : core.unit())(core.deferredSucceed(deferred, [value, winner])))(exit);
/* @internal */
...
const scope = /*#__PURE__*/Debug.methodWithTrace(trace => () => scopeTag.traced(trace));
/* @internal */
exports.scope = scope;
const scopeWith = /*#__PURE__*/Debug.methodWithTrace((trace, restore) => f => core.flatMap(scopeTag, restore(f)).traced(trace));
/* @internal */
...
const someWith = /*#__PURE__*/Debug.dualWithTrace(2, (trace, restore) => (self, f) => core.suspend(() => unsome(restore(f)((0, _effect.some)(self)))).traced(trace));
/* @internal */
...
const allPar = /*#__PURE__*/Debug.methodWithTrace(trace => function () {
  if (arguments.length === 1) {
    if (core.isEffect(arguments[0])) {
      ...
      return forEachPar(arguments[0], _Function.identity).traced(trace);
    } else {
  ...
  return forEachPar(arguments, _Function.identity).traced(trace);
});
/* @internal */
...
const validateAllPar = /*#__PURE__*/Debug.dualWithTrace(2, (trace, restore) => (elements, f) => core.flatMap(partitionPar(elements, restore(f)), ([es, bs]) => es.length === 0 ? core.succeed(bs) : core.fail(es)).traced(trace));
/* @internal */
exports.validateAllPar = validateAllPar;
const validateAllParDiscard = /*#__PURE__*/Debug.dualWithTrace(2, (trace, restore) => (elements, f) => core.flatMap(partitionPar(elements, restore(f)), ([es, _]) => es.length === 0 ? core.unit() : core.fail(es)).traced(trace));
/* @internal */
...
const withEarlyRelease = /*#__PURE__*/Debug.methodWithTrace(trace => self => scopeWith(parent => core.flatMap(core.scopeFork(parent, ExecutionStrategy.sequential), child => core.map(value => (0, RA.tuple)(core.fiberIdWith(fiberId => core.scopeClose(child, core.exitInterrupt(fiberId))), value))(scopeExtend(child)(self)))).traced(trace));
/* @internal */
...
const fiberCollectAll = fibers => ({
  ...
  children: Debug.methodWithTrace(trace => () => core.map(forEachPar(fibers, fiber => fiber.children()), RA.flatten).traced(trace)),
  inheritAll: Debug.methodWithTrace(trace => () => core.forEachDiscard(fibers, fiber => fiber.inheritAll()).traced(trace)),
  poll: Debug.methodWithTrace(trace => () => core.map(core.forEach(fibers, fiber => fiber.poll()), RA.reduceRight(Option.some(core.exitSucceed(new Array())), (optionB, optionA) => {
    switch (optionA._tag) {
      case "None":
        {
          return Option.none();
      ...
      case "Some":
        {
          switch (optionB._tag) {
            ...
            case "Some":
              {
                return Option.some(core.exitZipWith(optionB.value, (a, chunk) => [a, ...chunk], internalCause.parallel)(optionA.value));
              }
          }
        }
    }
...
/** @internal */
exports.race = race;
const disconnect = /*#__PURE__*/Debug.methodWithTrace(trace => self => core.uninterruptibleMask(restore => core.fiberIdWith(fiberId => core.flatMap(forkDaemon(restore(self)), fiber => core.onInterrupt(() => internalFiber.interruptAsFork(fiberId)(fiber))(restore(internalFiber.join(fiber)))))).traced(trace));
...
/** @internal */
const raceAwait = /*#__PURE__*/Debug.dualWithTrace(2, trace => (self, that) => core.fiberIdWith(parentFiberId => raceWith(that, (exit, right) => core.exitMatchEffect(cause => (0, _effect.mapErrorCause)(cause2 => internalCause.parallel(cause, cause2))(internalFiber.join(right)), value => core.as(value)(core.interruptAsFiber(parentFiberId)(right)))(exit), (exit, left) => core.exitMatchEffect(cause => (0, _effect.mapErrorCause)(cause2 => internalCause.parallel(cause2, cause))(internalFiber.join(left)), value => core.as(value)(core.interruptAsFiber(parentFiberId)(left)))(exit))(self)).traced(trace));
/** @internal */
